...
export default function PaymentUploadModalSimple({ participante, onClose, onSuccess }) {
    ...
    try {
      ...
      const registrosPagos = cuotasARegistrar.map(cuotaNum => {
        ...
        return {
          ...
          imagen_url: null, // Sin imagen por ahora
          estado: form.estado,
          notas: montoCompleto 
          ...
          <div className="flex flex-col items-center justify-center gap-4 py-8 text-center">
            ...
            <div className="bg-green-50 border border-green-200 rounded-lg p-3 mt-2">
              <div className="text-sm text-green-700">
                ...
                <div className="text-xs text-gray-600 mt-1">Nota: Sin imagen por problemas de configuración</div>
              </div>
            </div>
            <button
              onClick={onClose}
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="sticky top-0 bg-white border-b border-gray-100 p-6 flex items-center justify-between">
          <div>
            <h2 className="text-xl font-bold text-gray-800">Registrar Pago (Sin Imagen)</h2>
            <p className="text-sm text-gray-500 mt-1">
              {participante.nombre_apellido} • {participante.categoria}
            </p>
            <p className="text-xs text-amber-600 mt-1">
              ⚠️ Modo simple: Las imágenes están deshabilitadas temporalmente
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div className="bg-amber-50 border border-amber-200 rounded-lg p-4">
            <div className="font-semibold text-amber-800 mb-1">⚠️ Imágenes temporalmente deshabilitadas</div>
            <div className="text-sm text-amber-700">
              <p className="mb-2">El sistema de almacenamiento de imágenes necesita configuración adicional.</p>
              <p className="mb-2">Para habilitar imágenes:</p>
              <ol className="list-decimal pl-5 space-y-1">
                <li>Ejecuta <code className="bg-amber-100 px-1 rounded">simple_storage_fix.sql</code> en Supabase</li>
                <li>Configura CORS en el dashboard de Supabase Storage</li>
                <li>Luego podrás subir imágenes normalmente</li>
              </ol>
            </div>
          </div>
